    ...
    while True:
        ...
        for i in range(10, -1, -1):
            for j, var in enumerate(variables):
                condition_guess[str(var)] = (
                    current_guess[str(var)] + (delta[j] / 2**i)
                ).evalf()
            f_norm_condition = Matrix(functions).subs(condition_guess).norm()

            print(f_norm_condition.evalf())
            print(Matrix(functions).subs(condition_guess).evalf())
        ...
        for j, var in enumerate(variables):
            current_guess[str(var)] = (
                current_guess[str(var)] + (delta[j] / 2**k)
            ).evalf()
