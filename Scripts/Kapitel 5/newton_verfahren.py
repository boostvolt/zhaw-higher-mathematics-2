...
def newton_method(functions, initial_guess, iterations, debug=False):
    ...
    for i in range(iterations):
        ...
        delta = -jacobian_matrix.inv().subs(substitutions) * Matrix(functions).subs(substitutions)

        previous_guess = current_guess.copy()
        current_guess = {str(var): current_guess[str(var)] + delta[j] for j, var in enumerate(variables)}

        f_norm = Matrix([f.subs(substitutions) for f in functions]).norm()
        x_norm = Matrix([current_guess[str(var)] - previous_guess[str(var)] for var in variables]).norm()

        if debug:
            print(f"Iteration {i+1}: x = {current_guess}, ||f(x^(k))|| = {f_norm}, ||x(k) - x(k-1)|| = {x_norm}")
